...
func chunk_offset_to_player(chunk_delta: Vector2i) -> Vector2:
    var player_cell_pos = player.global_position / PIXELS_PER_CELL
...
func _process(delta: float) -> void:
    for dx in range(-CHUNKS_TO_LOAD, CHUNKS_TO_LOAD):
        for dy in range(-CHUNKS_TO_LOAD, CHUNKS_TO_LOAD):
            ...
            var master_cell = chunk_offset_to_player(chunk_delta)
    ...
    for dx in range(CHUNK_SIZE):
        for dy in range(CHUNK_SIZE):
            ...
            if noise.get_noise_2d(cell.x, cell.y) > -0.3:
                $Cliffs.set_cell(cell, 0, AIR)
            else:
                $Cliffs.set_cell(cell, 0, GRASS)
